...
https://api.blazon.build/v1/mark/1.4.2.png?renderer=bishop&size=112
```
...
**Prefer SVG.** It is the format the service is comfortable with: its cost is
whatever the renderer's own maths costs and does not grow with the requested
size, so it is served at any size up to 1024 with no per-renderer limit.

**PNG is capped**, because rasterising and deflating cost the square of the
edge length and the deployment has ten milliseconds of CPU per request. The
ceilings are measured, published at `/v1/renderers`, and currently run from 48
to 112 pixels depending on the renderer. `flowfield` is not served as PNG at
all: its streamline integration costs the same at sixteen pixels as at
five hundred, so there is no size at which it fits.

These are limits of one small deployment, not of the library. `blazon.PNG` has
